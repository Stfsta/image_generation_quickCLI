...
- **运行时语言切换**：支持 `lang en|zh`，并自动持久化到 `config.json`
- **更便携参考图导入**：将参考图放入 `reference_images/`，在未显式指定时自动识别并使用
- 轻量、易读的代码结构，方便二次开发
...
英文模式提示符：`Prompt/Command >`，中文模式提示符：`提示词/命令 >`。
...
`gpt-image-2` 模型支持图生图（Image-to-Image）功能。你可以提供参考图像，配合文本提示词来引导生成过程。脚本支持**四种语法风格**：

#### 方式一：内联语法（推荐）

直接在提示词中包含图片引用：

`提示词/命令 > [image:/path/to/reference_image.jpg] 把背景改成日落场景`

#### 方式二：命令行参数语法

在提示词前使用 `--ref` 参数：

`提示词/命令 > --ref /path/to/reference_image.jpg 把背景改成日落场景`

#### 方式三：会话级参考图片

设置一张参考图片，在当前会话中后续所有生成都使用该图片，直到执行 `ref clear` 或切换会话：

`提示词/命令 > ref /path/to/reference_image.jpg`  
`提示词/命令 > 把背景改成日落场景`  
`提示词/命令 > 多加一些云`  
`提示词/命令 > ref clear`
...
- 若识别到的文件全部为本地 PNG 且都 <= 4MB，则自动走 `/v1/images/edits`。
- 否则自动回退到 `/v1/images/generations`。
- 若目录不存在或为空，则按原有文生图流程执行。

你也可以使用 URL 作为参考图片：

`提示词/命令 > --ref https://example.com/reference.png 给这张图加上雪景效果`
...
`提示词/命令 > [size:1792x1024] 一幅全景山水画`

也可使用临时 CLI 参数语法：

`提示词/命令 > --size 1024x1792 一位竖版构图的幻想角色`
...
| `ref clear`           | 清除会话级参考图片  |
| `lang [en/zh]`        | 查看或切换 CLI 显示语言 |
| `language [en/zh]`    | `lang` 的别名 |
| `help`                | 显示帮助信息     |
...
| `language`     | CLI 显示语言（`en` / `zh`）                                                                          | `en`                                             |
| `image_dir`    | 图片保存目录                                                                                         | `./generated_images`                             |
| `reference_dir`| 当未显式指定参考图时自动扫描的参考图目录                                                                     | `./reference_images`                             |
